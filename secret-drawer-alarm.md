...
First, we need our gadget to wait for the light to change. Go to the **INPUT** drawer and pull out the `||input:on light bright||` block. This block will run whatever code we put inside it the moment the sensor detects light.
...
Now for the sound. Go to the **MUSIC** drawer and grab `||music:play tone High A for 1/2 beat||`. Snap it right under the red light block. 
...
Go to the **MUSIC** drawer one more time. Grab `||music:play tone High E for 1/2 beat||` and snap it at the bottom of your loop. 

## Step 7: Turn it Off
When the alarm finishes repeating 5 times, it should turn the lights off so it can hide in the dark again. Go to the **LIGHT** drawer, grab a `||light:clear||` block, and snap it at the very bottom of your `||input:on light bright||` block, *outside* the loop.
